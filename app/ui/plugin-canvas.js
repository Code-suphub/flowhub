(() => {
  ...
  function minimum(card){const w=sources.find(s=>s.id===card.plugin)?.widget;return {width:Math.max(w?.interactive?320:160,Math.min(1600,Number(w?.minWidth)||0)),height:Math.max(w?.interactive?280:120,Math.min(1200,Number(w?.minHeight)||0))};}
  function render(){
    // Cache refreshes must not tear down a card while its controls or confirmation are active.
    for(const c of board().cards){const min=minimum(c),interactive=sources.find(s=>s.id===c.plugin)?.widget?.interactive;c.width=Math.max(min.width,c.width||({small:184,medium:384,large:384}[c.size])||184);c.height=Math.max(min.height,c.height||(interactive?360:({small:184,medium:184,large:384}[c.size]))||184);}
    const structure=JSON.stringify([layout,sources.map(s=>({id:s.id,title:s.title,widget:s.widget,lastLoadedAt:s.lastLoadedAt}))]);
    ...
    // Position initialization may mutate layout; cache the final structure so
    // the next metrics refresh does not unnecessarily rebuild every iframe.
    renderedStructure=JSON.stringify([layout,sources.map(s=>({id:s.id,title:s.title,widget:s.widget,lastLoadedAt:s.lastLoadedAt}))]);
  }
  ...
  $('#canvas').addEventListener('pointermove',e=>{
    ...
    if(g.resize){const min=minimum(board().cards.find(c=>c.id===g.id));g.node.style.width=Math.round(Math.max(min.width,Math.min(1600,dx+g.width)))+'px';g.node.style.height=Math.round(Math.max(min.height,Math.min(1200,dy+g.height)))+'px';}
    else {g.node.classList.add('moving');g.wanted={x:Math.max(0,g.left+dx),y:Math.max(0,g.top+dy)};g.node.style.left=g.wanted.x+'px';g.node.style.top=g.wanted.y+'px';const canvasRect=$('#canvas').getBoundingClientRect();g.target=WidgetLayout.swapTarget(board().cards,g.id,{x:e.clientX-canvasRect.left,y:e.clientY-canvasRect.top});for(const n of document.querySelectorAll('.card'))n.classList.toggle('swap-target',n.dataset.id===g.target);if(invoke&&Date.now()-(g.grown||0)>150&&(e.clientX>innerWidth-40||e.clientY>innerHeight-40)){g.grown=Date.now();fitted='';invoke('plugin_canvas_api',{action:'fit',payload:{width:innerWidth+(e.clientX>innerWidth-40?160:0),height:innerHeight+(e.clientY>innerHeight-40?140:0)}}).catch(()=>{});}}

  });
  const finish=(e,cancel=false)=>{if(!gesture)return;const g=gesture;gesture=null;for(const n of document.querySelectorAll('.swap-target'))n.classList.remove('swap-target');g.node.classList.remove('moving');if(g.node.hasPointerCapture(e.pointerId))g.node.releasePointerCapture(e.pointerId);if(!cancel&&g.moved){suppressClick=true;setTimeout(()=>suppressClick=false,100);const items=board().cards,card=items.find(c=>c.id===g.id);if(g.resize){card.width=parseInt(g.node.style.width);card.height=parseInt(g.node.style.height);}else {card.width=g.width;card.height=g.height;positionCards(card.id,g.wanted,g.target);}commit();}else if(cancel){renderedStructure='';render();}};
  $('#canvas').addEventListener('pointerup',e=>finish(e));$('#canvas').addEventListener('pointercancel',e=>finish(e,true));
  $('#canvas').addEventListener('keydown',e=>{if(!e.target.matches('[data-resize]')||!['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key))return;e.preventDefault();const node=e.target.closest('.card'),card=board().cards.find(c=>c.id===node.dataset.id),rect=node.getBoundingClientRect(),min=minimum(card);card.width=Math.max(min.width,Math.min(1600,(card.width||rect.width)+(e.key==='ArrowRight'?10:e.key==='ArrowLeft'?-10:0)));card.height=Math.max(min.height,Math.min(1200,(card.height||rect.height)+(e.key==='ArrowDown'?10:e.key==='ArrowUp'?-10:0)));commit();document.querySelector(`[data-id="${CSS.escape(card.id)}"] [data-resize]`)?.focus();});
