...
fn enabled(state: &AppState) -> bool {
    ...
    fs::read_to_string(config).ok().and_then(|v| serde_json::from_str::<Value>(&v).ok()).and_then(|v| v.get("enabled").and_then(Value::as_bool)).unwrap_or(false)
}

fn snapshot() -> Value {
    let output = Command::new("ps").args(["-axo", "pid=,ppid=,%cpu=,%mem=,rss=,etime=,command="]).output().ok();
    let processes = output.map(|value| String::from_utf8_lossy(&value.stdout).lines().filter(|line| line.to_lowercase().contains("flowhub") || line.to_lowercase().contains("webkit")).map(|line| json!({"raw": line.trim()})).collect::<Vec<_>>()).unwrap_or_default();
...
pub fn set_diagnostics_enabled(state: State<'_, AppState>, value: bool) -> Result<Value, String> {
    ...
    fs::write(&config, serde_json::to_vec_pretty(&json!({"enabled": value, "updatedAt": chrono::Utc::now().to_rfc3339()})).map_err(|e| e.to_string())?).map_err(|e| e.to_string())?;
    Ok(json!({"ok": true, "enabled": value, "path": log}))
}

#[tauri::command]
pub fn sample_diagnostics(state: State<'_, AppState>) -> Result<Value, String> {
    if !enabled(&state) { return Ok(json!({"ok": false, "disabled": true})); }
    let (log, _) = paths(&state);
    if let Some(parent) = log.parent() { fs::create_dir_all(parent).map_err(|e| e.to_string())?; }
    let record = snapshot();
    let mut file = OpenOptions::new().create(true).append(true).open(&log).map_err(|e| e.to_string())?;
...
pub fn clear_diagnostics(state: State<'_, AppState>) -> Result<Value, String> {
    let (log, _) = paths(&state);
    if log.exists() { fs::write(&log, "").map_err(|e| e.to_string())?; }
