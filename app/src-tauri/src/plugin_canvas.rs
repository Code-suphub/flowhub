...
pub fn plugin_canvas_api(window:tauri::WebviewWindow,app:tauri::AppHandle,action:String,payload:Value)->Result<Value,String>{
    ...
    let sources:Vec<_>=app.state::<crate::plugin_runtime::Runtime>().status_plugins().into_iter().map(|(id,title)|json!({"snapshot":app.state::<crate::plugin_status::State>().snapshot(&id),"widget":app.state::<crate::plugin_runtime::Runtime>().widget(&id).ok(),"id":id,"title":title})).collect();
