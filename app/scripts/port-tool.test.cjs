...
(async()=>{
 ...
 let resolveInspect;const before=inspections;
 context.api.inspectPort=()=>{inspections++;return new Promise(resolve=>resolveInspect=resolve);};
 const first=reg.action('port','refresh',{dataset:{}},context);
 const second=reg.action('port','refresh',{dataset:{}},context);
 await Promise.resolve();assert.equal(inspections,before+1);
 resolveInspect({port:9000,processes:[process]});await Promise.all([first,second]);
 reg.queryChanged({...context,enabled:()=>false});assert.equal(reg.suggestions({...context,enabled:()=>false}).length,0);
 console.log('PASS: port bounds/injection, inspection, confirmation/cancel, identity forwarding, Linux command copy, refresh deduplication, disabled tool');
