...
const rendererUrl = process.env.WEBORG_RENDERER_URL || "http://127.0.0.1:5173";
...
try {
  ...
  console.log(`[weborg] 浏览器预览：${rendererUrl}/search.html`);
  console.log(`[weborg] 配置页预览：${rendererUrl}/settings.html`);
  const electron = run(electronPath, [appDirectory, "--dev"], {
    env: { ...process.env, WEBORG_RENDERER_URL: rendererUrl }
  });
  electron.once("exit", (code) => stop(code || 0));
} catch (error) {
  console.error(`[weborg] 开发模式启动失败：${error.message}`);
  stop(1);
}
